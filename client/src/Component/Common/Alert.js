...
export const ErrorAlert = ( { visible, setVisible, message } ) => {
  ...
  return (
    <div className='absolute top-4 right-4 z-[1000]'>
      <div
        className={`bg-teal-100 border-t-4 border-red-500 rounded-b text-teal-900 px-4 py-3 shadow-md transition-opacity duration-500 ease-in-out ${visible ? 'opacity-100' : 'opacity-0'}`}
        role="alert"
      >
        <div className="flex gap-4 justify-between">
          <div className="flex items-center">
          ...
          <button onClick={hideAlert} className="text-red-500 font-bold">&times;</button>
        </div>
      </div>
    </div>
  );
